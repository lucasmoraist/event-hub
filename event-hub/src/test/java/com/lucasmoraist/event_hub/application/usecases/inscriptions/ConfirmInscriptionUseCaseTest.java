...
class ConfirmInscriptionUseCaseTest {
    ...
    @Test
    @DisplayName("Should confirm an inscription successfully")
    void case01() {
        ...
        InscriptionResponse response = confirmInscriptionUseCase.execute(inscriptions.getId());

        assertAll("Response",
                () -> assertNotNull(response),
                () -> assertEquals(inscriptions.getId(), response.id()),
                () -> assertEquals(user.getName(), response.userName()),
                () -> assertEquals(user.getEmail(), response.userEmail()),
                () -> assertEquals(event.getTitle(), response.eventTitle()),
                () -> assertEquals(event.getDate().toString(), response.eventDate()),
                () -> assertEquals(event.getTime().toString(), response.eventTime()),
                () -> assertEquals(event.getLocation(), response.eventLocation()),
                () -> assertEquals(StatusInscriptions.CONFIRMED.name(), response.status())
        );

        assertAll("Verifier",
                () -> verify(inscriptionRepository, times(1))
                        .save(inscriptions),
                () -> verify(getInscriptionById, times(1))
                        .execute(inscriptions.getId()),
